    ...
    if ($row = $result->fetch_assoc()) {
        ...
        if (empty($khoahoc_ids[0])) {
           ...
           // Trong phần kiểm tra hoàn thành khóa học, sửa lại như sau:
            $stmt_completed = $conn->prepare("SELECT 
                ...
                COUNT(DISTINCT kq.test_id) as attempted_tests
                FROM kiem_tra kt
            ...
            $status = $is_completed ? "<span class='completed'>Hoàn thành </span>" : "<span class='not-completed'>Chưa hoàn thành</span>";
                echo "<li>
                    ...
                    <div class='test-info'>
                        <span>Đã hoàn thành: $total_tests/$total_tests bài test</span>
                    </div>
...
if (isset($_GET['khoa_hoc_id'])) {
    ...
    if ($result3->num_rows > 0) {
        while ($test = $result3->fetch_assoc()) {
            ...
            $kq_cao_nhat = $ketqua_data['kq_cao_nhat'] ?? 'Chưa có';
            $tt_bai_test = $ketqua_data['tt_bai_test'] ?? '';

        
            // Đếm số lần thử từ tt_bai_test
            $so_lan_thu = 0;
            $stmt_attempts = $conn->prepare("SELECT Trial FROM kiem_tra WHERE Student_ID = ? AND Khoa_ID = ? AND Test_ID = ?");
            $stmt_attempts->bind_param("sis", $student_id, $khoa_hoc_id, $test_id);
            $stmt_attempts->execute();
            $attempts_result = $stmt_attempts->get_result();
            if ($attempts_row = $attempts_result->fetch_assoc()) {
                $so_lan_thu = $attempts_row['Trial'];
            }     
            ...
            $kq_cao_nhat = 0;
            if (is_numeric($kq_cao_nhat)) {
                $so_cau_dung = round(($kq_cao_nhat / 100) * $total_questions);
            }

            // Kiểm tra xem bài test đã đậu chưa (dựa trên số câu đúng)
            $is_passed = is_numeric( $kq_cao_nhat) && ($so_cau_dung >= round(($pass_score / 100) * $total_questions));
            $passed_status = $is_passed ? "<span class='passed'>Đạt - $so_cau_dung/$total_questions câu</span>" : 
                            (is_numeric( $kq_cao_nhat) ? "<span class='not-completed'>Chưa đạt - $so_cau_dung/$total_questions câu</span>" : 
            ...
            echo "<li>
                ...
                <div class='test-info'>
                    <span>Điểm cao nhât: $kq_cao_nhat ($pass_score%)</span>
