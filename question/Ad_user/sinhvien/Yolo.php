            ...
            <?php else: ?>
                <?php foreach ($_SESSION['questions'] as $index => $question): ?>
                    <div class="question-block">
                        ...
                        <p class="question-text">Câu <?php echo $index +1; ?><?php echo htmlspecialchars ($question ['question']);?>
                        <?php if (!empty($question['image'])): ?>
                            <img src="<?php echo htmlspecialchars($question['image']); ?>" alt="Hình ảnh câu hỏi">
                        <?php endif; ?>
                        <ul>
...
<script>
    // Kiểm tra nếu không có trạng thái nào trước đó, đẩy một entry giả vào lịch sử
    if (window.performance && performance.navigation.type === 1) {
        // Người dùng refresh trang — không cần xử lý gì
    } else {
        // Người dùng vào từ ngoài (VD: Google)
        history.replaceState({ isFake: true }, null, location.href); // Thay trang hiện tại
        history.pushState({ isReal: true }, null, location.href);    // Thêm trang "giả"
    }

    window.onpopstate = function(event) {
        if (event.state && event.state.isFake) {
            // Khi quay lại entry đầu tiên (giả), chuyển hướng đến $link_quay_lai
