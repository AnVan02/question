...
if (isset($_GET['xem_ket_qua'])) {
    ...
    // Lấy kết quả từ bảng ket_qua (đã là kết quả cao nhất)
    $stmt_ketqua = $conn->prepare("SELECT kq_cao_nhat, tt_bai_test FROM ket_qua 
                                   WHERE student_id = ? AND khoa_id = ? AND test_id = ?");
    $stmt_ketqua->bind_param("iis", $student_id, $khoa_hoc_id, $test_id);
    $stmt_ketqua->execute();
    ...
    if ($ketqua_data) {
        ...
        echo "<h3>Kết quả cao nhất bài test: <strong>$test_name</strong> (Môn: $khoa_name)</h3>";
        echo "<div class='test-result'>";
        echo "<p><strong>Điểm cao nhất:</strong> " . htmlspecialchars($kq_cao_nhat) . "</p>";
        
        // Phân tích tt_bai_test để lấy đáp án của lần làm bài có điểm cao nhất
        $user_answers = [];
        ...
        if ($quiz_result->num_rows == 0) {
            ...
            $question_number =1;
            $index =1; // sử dụng index để ánh xạ theo thứ thự nếu id_cauhoi không khớp
            while ($q = $quiz_result->fetch_assoc()) {
                $cau_id = $q['Id_cauhoi'];
                $user_answer = isset($user_answers[$cau_id]) ? $user_answers[$cau_id] : null;
                $dap_an_dung = strtoupper($q['dap_an']);
                
                echo "<div class='question-container'>";
                echo "<div><strong>Câu {$q['Id_cauhoi']}:</strong> {$q['cauhoi']}</div>";
                echo "<div class='options'>";
                ...
                foreach ($options as $key => $value) {
                    ...
                    echo "<div class='option $class'>";
                    echo "<strong>$key.</strong> $value $icon";
                    echo "</div>";
                }
