...
if (isset($_GET['student_id']) && !empty($_GET['student_id'])) {
    ...
    if ($row = $result->fetch_assoc()) {
        ...
        if (empty($khoahoc_ids[0])) {
            ...
            $passed_tests = $completed_data['passed_tests'] ?? 0;

            // Chỉ hiển thị trạng thái nếu đã có bài test được gán
            if ($total_tests > 0) {
                // Nếu đã làm ít nhất 1 bài test
                if ($attempted_tests > 0) {
                    $is_completed = ($passed_tests == $total_tests);

                $status = $is_completed 
                    ? "<span class='completed'>Hoàn tất</span>" 
                    : "<span class='not-completed'>Chưa hoàn thành</span>";          
                 } 
                // Nếu chưa làm bài test nào
                else {
                    $status = ""; // Không hiển thị gì nếu chưa thi
                }
            echo "<li>
            ...
            // Chỉ hiển thị thông tin bài test nếu đã có bài test được làm
            if ($attempted_tests > 0) {
                echo "<div class='test-info'>
                    <span>Đã làm: $attempted_tests / $total_tests bài test</span>
                    </div>";
            }
            // chỉ hiên thị thông tin bài test lấy chi tiết hoàn tất
            
            if($total_completer > 0) {
                if($attempted_tests == $total_tests && $Passed_tests == $total_tests) {
                    $status = "<span class ='completed'>Hoàn tất</span>";
                }else {
                    $status = "<span class=not-completed'>Chưa đạt";
                }
            ...
            if ($total_tests > 0) {
                if ($attempted_tests == $total_tests && $passed_tests == $total_tests) {
                    ...
                    $status = "<span class='not-completed'>Chưa hoàn thành</span>";
                }
            } else {
                $status = "<span class='not-completed'>Chưa hoàn thành</span>";
            }
            ...
            // Nếu không có bài test nào được gán
            echo "<li>
                <div class='course-header'>
                    <span><strong>{$row2['khoa_hoc']}</strong></span>
                </div>
...
if (isset($_GET['khoa_hoc_id'])) {
    ...
    if ($result3->num_rows > 0) {
        while ($test = $result3->fetch_assoc()) {
            ...
            $percentage = is_numeric($diem_cao_nhat) ? round($diem_cao_nhat) : 0;
    ...
    if ($ketqua_data) {
        ...
        if (!empty($tt_bai_test)) {
            if (preg_match_all('/Câu\s+(\d+):\s*([A-D])/', $tt_bai_test, $matches, PREG_SET_ORDER)) {
                ...
                echo "<div class='error'>Không thể phân tích đáp án. Định dạng không khớp.</div>";
            }
        } else {
            echo "<div class='error'>Đáp án không được lưu vào database .</div>";
        }
