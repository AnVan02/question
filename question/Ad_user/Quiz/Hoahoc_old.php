...
if ($result->num_rows === 0) {
    die("ID bài test ($id_test) không tồn tại trong hệ thống. Vui lòng kiểm tra lại!");
}
...
// Truy vấn để lấy thông tin student_id từ bảng students
$sql = "SELECT Student_ID, khoa_hoc FROM students WHERE Student_ID, khoa_hoc = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("ii", $student_id, $khoa_hoc);
$stmt->execute();
$result = $stmt->get_result();
