    ...
    <div class="container">
        ...
        <?php else: ?>
           ...
           // Lưu kết quả vào bảng ket_qua
            ...
            // Kiểm tra xem đã có kết quả nào chưa
            $check_sql = "SELECT kq_cao_nhat FROM ket_qua 
                        WHERE student_id = ? AND khoa_id = ? AND test_id = ?";
            $check_stmt = $conn->prepare($check_sql);
            $check_stmt->bind_param("sis", $student_id, $ma_khoa, $id_test);
            $check_stmt->execute();
            $check_result = $check_stmt->get_result();

            if ($check_result->num_rows > 0) {
                // Đã có bản ghi - thực hiện UPDATE
                $row = $check_result->fetch_assoc();
                $existing_score = $row['kq_cao_nhat'];
                
                // Chỉ cập nhật nếu điểm mới cao hơn
                if ($highest_score > $existing_score) {
                    $update_sql = "UPDATE ket_qua SET 
                                kq_cao_nhat = ?, 
                                tt_bai_test = ?
                                WHERE student_id = ? AND khoa_id = ? AND test_id = ?";
                    $update_stmt = $conn->prepare($update_sql);
                    $update_stmt->bind_param("issis", $highest_score, $tt_bai_test, 
                                        $student_id, $ma_khoa, $id_test);
                    $update_stmt->execute();
                    $update_stmt->close();
                } else {
                    // Chỉ cập nhật tt_bai_test nếu điểm không cao hơn
                    $update_sql = "UPDATE ket_qua SET 
                                tt_bai_test = ?
                                WHERE student_id = ? AND khoa_id = ? AND test_id = ?";
                    $update_stmt = $conn->prepare($update_sql);
                    $update_stmt->bind_param("siss", $tt_bai_test, 
                                        $student_id, $ma_khoa, $id_test);
                    $update_stmt->execute();
                    $update_stmt->close();
                }
            } else {
                // Chưa có bản ghi - thực hiện INSERT
                $insert_sql = "INSERT INTO ket_qua 
                            (student_id, khoa_id, test_id, kq_cao_nhat, tt_bai_test)
                            VALUES (?, ?, ?, ?, ?)";
                $insert_stmt = $conn->prepare($insert_sql);
                $insert_stmt->bind_param("isiss", $student_id, $ma_khoa, $id_test, 
                                    $highest_score, $tt_bai_test);
                
                // Xử lý ngoại lệ nếu INSERT vẫn bị lỗi
                try {
                    $insert_stmt->execute();
                } catch (mysqli_sql_exception $e) {
                    // Nếu lỗi duplicate, chuyển sang UPDATE
                    if ($e->getCode() == 1062) {
                        $update_sql = "UPDATE ket_qua SET 
                                    kq_cao_nhat = ?, 
                                    tt_bai_test = ?
                        ...
                        // Nếu lỗi khác, hiển thị thông báo
