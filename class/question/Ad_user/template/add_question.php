    ...
    if ($image && file_exists($image)) {
        try {
            ...
            if (unlink($image)) {
                ...
                // Cập nhật database
                $stmt = $conn->prepare("UPDATE quiz SET hinhanh = NULL WHERE Id_cauhoi = ?");
                ...
                $_SESSION['message'] = "Đã xóa ảnh thành công";
            } else {
            ...
            $_SESSION['error'] = "Có lỗi xảy ra khi xóa ảnh: " . $e->getMessage();
        }
    } else {
        $_SESSION['error'] = "File ảnh không tồn tại";
    }
    
    // Chuyển hướng trở lại trang trước
    header("Location: add_question.php?id=" . $id);
    exit();
}
...
<head>
    ...
    function updateBaiTestOptions() {
        ...
        if (selectedKhoa == '<?= $test['id_khoa'] ?>') {
            ...
            option.value = '<?= $test['ten_test'] ?>';
            option.textContent = '<?= htmlspecialchars($test['ten_test']) ?>';
            <?php if(isset($question_data['id_baitest']) && $question_data['id_baitest'] == $test['ten_test']): ?>
            ...
            <div class="form-row">
                ...
                <div>
                    ...
                    <select name="id_baitest" id="id_baitest" required>
                        ...
                        <?php foreach($khoa_hoc_list as $khoa): ?>
                            <option value="<?= $khoa['id_baitest'] ?>" 
                                <?php 
                                // Tìm id_khoa từ question_data nếu đang edit
                                if($question_id && !empty($question_data['ten_test'])) {
                                    foreach($khoa_hoc_list as $k) {
                                        if($k['id_baitest'] == $question_data['ten_test'] && $k['id_baitest'] == $khoa['id_baitest']) {
                                            echo 'selected';
                                            break;
                                        }
                                    }
