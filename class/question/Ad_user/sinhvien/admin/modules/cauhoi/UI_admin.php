...
function display_question_answers($user_answers, $conn, $test_name, $khoa_name) {
    ...
    $quiz_result = $conn->query("
        SELECT Id_cauhoi, cauhoi, cau_a, cau_b, cau_c, cau_d, dap_an, 
               giaithich_a, giaithich_b, giaithich_c, giaithich_d,
               hinhanh, hinhanh_a, hinhanh_b, hinhanh_c, hinhanh_d
        FROM quiz 
        WHERE id_baitest = '$test_name' AND ten_khoa = '$khoa_name' 
        ORDER BY Id_cauhoi
    ");
    ...
    if ($quiz_result && $quiz_result->num_rows > 0) {
        ...
        while ($q = $quiz_result->fetch_assoc()) {
            ...
            echo "<div class='question-container'>";
            echo "<p><strong>Câu $question_number</strong> <span class='question-id'>(ID: $question_id)</span>: " . htmlspecialchars($q['cauhoi']);
            if (!empty($q['hinhanh'])) {
                echo "<br><img src='" . htmlspecialchars($q['hinhanh']) . "' alt='Ảnh câu hỏi' style='max-width:300px;max-height:200px;margin-top:5px;'>";
            }
            echo "</p>";
            
            echo "<div class='options'>";
            $choices = [
                'A' => ['text' => $q['cau_a'], 'img' => $q['hinhanh_a'] ?? ''],
                'B' => ['text' => $q['cau_b'], 'img' => $q['hinhanh_b'] ?? ''],
                'C' => ['text' => $q['cau_c'], 'img' => $q['hinhanh_c'] ?? ''],
                'D' => ['text' => $q['cau_d'], 'img' => $q['hinhanh_d'] ?? '']
            ];
            ...
            foreach ($choices as $key => $value) {
                ...
                echo "<div class='option $class'>";
                echo $key . ". " . htmlspecialchars($value['text']);
                if (!empty($value['img'])) {
                    echo "<br><img src='" . htmlspecialchars($value['img']) . "' alt='Ảnh đáp án' style='max-width:300px;max-height:200px;margin-top:5px;'>";
                }
                echo " $icon";
                if ($is_correct) {
                    echo " <span>(Đáp án đúng)</span>";
