...
// Lấy id_test từ URL và đảm bảo là số nguyên
...
if (isset($_GET['delete_id']) && is_numeric($_GET['delete_id']) && $id_test > 0) {
    ...
    if ($result->num_rows > 0) {
        ...
        $sql = "SELECT hinhanh, hinhanh_a, hinhanh_b, hinhanh_c, hinhanh_d 
                FROM quiz 
                WHERE Id_cauhoi = ? AND id_baitest = ? AND ten_khoa = ?";
        $stmt = $conn->prepare($sql);
        ...
        if ($result->num_rows > 0) {
            $row = $result->fetch_assoc();
            // Xóa các hình ảnh nếu tồn tại
            foreach (['hinhanh', 'hinhanh_a', 'hinhanh_b', 'hinhanh_c', 'hinhanh_d'] as $img_field) {
...
if ($id_test > 0 && $test_info && $khoa_hoc) {
    $conn = dbconnect();
    $sql = "SELECT q.Id_cauhoi, q.id_baitest, q.ten_khoa, q.cauhoi, q.hinhanh, 
                   q.cau_a, q.hinhanh_a, q.cau_b, q.hinhanh_b, q.cau_c, q.hinhanh_c, 
                   q.cau_d, q.hinhanh_d, q.dap_an 
            FROM quiz q 
            JOIN test t ON q.id_baitest = t.id_test 
            JOIN khoa_hoc k ON q.ten_khoa = k.khoa_hoc 
            WHERE t.id_test = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $id_test);
    $stmt->execute();
...
<body>
    <div class="container">
        <h2>Danh sách câu hỏi: <?php echo htmlspecialchars($test_info['ten_test'] ?? 'Không xác định'); ?> 
                            ...
                            <tr>
                                ...
                                <td>
                                    <ul class="answer-list">
                                        <li><?php echo htmlspecialchars($question['cauhoi']); ?>
                                            <?php if (!empty($question['hinhanh'])): ?>
                                                <br><img src="<?php echo htmlspecialchars($question['hinhanh']); ?>" alt="Hình ảnh câu hỏi">
                                            <?php endif; ?>
                                        </li>
                                    </ul>
                                    <ul class="answer-list">
                                ...
                                <td>
                                    ...
                                    <a href="index.php?action=question&id_test=<?php echo htmlspecialchars($id_test); ?>&delete_id=<?php echo htmlspecialchars($question['Id_cauhoi']); ?>" 
                                       class="btn-delete" 
