...
if ($question_id) {
    $conn = dbconnect();
    $sql = "SELECT * FROM khoahoc WHERE Id = ?";
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["save_question"])) {
    ...
    if (empty($id_baitest) || empty($question_text) || empty($choices['A']) || empty($choices['B']) ||
        ...
        $conn = dbconnect();
        if ($question_id) {
            $sql = "UPDATE quiz SET id=? , ten_khoahoc =?
                    WHERE Id_cauhoi=?";
            $stmt = $conn->prepare($sql);
        ...
        <div class="container">
            ...
            <?php if (!empty($message)) echo $message; ?>
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <label>Thêm khoá </label><br>
                    <textarea name="question_text" rows="4" cols="50"><?= htmlspecialchars($question_data['id_khoahoc'] ?? '') ?></textarea><br><br>
                    ...
                    <div>
                        <button type="submit" name="save_question" class="btn btn-primary">Lưu lại</button>
                        <!-- <button type="button" onclick="window.location.href='question.php'" class="btn btn-secondary">Danh sách câu hỏi</button> -->
                    </div>

                    <?php foreach ($questions as $question): ?>
                        <tr>
                            <td><?= htmlspecialchars ($question['id_khoahoc'])?></td>
                            <td><?= htmlspecialchars ($question['ten_khoahoc']) ?></td>
                        </tr>
                        <td>
                            <a href="add_khoahoc.php?khoahoc_id=<?=$khoahoc ['id_khoahoc']?>"class="btn-edit">Sửa</a>
                            <a href="khoahoc.php?delete_id =<?= $khoahoc ['Id_khoahoc'] ?>"
                            class="btn-delete"
                            onclick="return confirm ('bạn có chắc chắn muôn xoá câu hỏi này không ?');>Xoá</a>
