...
if (!isset($_GET['test_id']) || !is_numeric($_GET['test_id'])) {
    echo "<script>
        alert('ID bài test không hợp lệ!');
...
// Lấy id_khoa từ id_test
$stmt = $conn->prepare("SELECT id_khoa, ten_test FROM test WHERE id_test = ?");
...
// Chuyển chuỗi Khoahoc thành mảng và kiểm tra
$khoahoc_array = explode(',', $khoahoc);
if (!in_array($ma_khoa, $khoahoc_array)) {
    echo "<script>
...
if ($row = $result->fetch_assoc()) {
    ...
    if (count($questions) < 1) {
        die("Lỗi: Không có câu hỏi nào cho '$ten_khoa' và '$id_baitest'.");
    }
