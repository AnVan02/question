    ...
    <div class="main-content">
                ...
                <form method="POST" action="<?php p($_['del_url']); ?>" id="delete-form">
                    <table id="employeeTable" class="employee-table">
                        ...
                        <?php foreach ($_['data'] as $index => $row): ?>
                            <tr data-employee-id="<?php echo htmlspecialchars($row[0]); ?>" data-employee-name="<?php echo htmlspecialchars($row[1]); ?>" data-nextcloud-account="<?php echo htmlspecialchars($row[2]); ?>" data-position="<?php echo ($row[3] == 1) ? 'manager' : ''; ?>">
                                <td>
                                    <input type="checkbox" name="emp_ids[]" value="<?php echo htmlspecialchars($row[0]); ?>" class="row-checkbox">
                                </td>
                                <td><?php echo $index + 1; ?></td>
                                <td><?php echo htmlspecialchars($row[0]); ?></td>
                                <td><?php echo htmlspecialchars($row[1]); ?></td>
                                <td><?php echo htmlspecialchars($row[2]); ?></td>
                                <td><?php echo ($row[3] == 1) ? 'Quản lý' : 'Nhân viên'; ?></td>
                            </tr>
                        <?php endforeach; ?>
                        </tbody>
                    </table>
        ...
        <div id="addEmployeePopup" class="popup" style="display: none;">
            <div class="popup-content">
                ...
                <form id="addEmployeeForm" method="POST" action="<?php p($_['add_url']); ?>" enctype="multipart/form-data">
                    ...
                    <input type="file" id="image" name="images[]" accept="image/*" webkitdirectory directory multiple required>
        ...
        <div id="editEmployeePopup" class="popup" style="display: none;">
            <div class="popup-content">
                ...
                <form method="POST" action="<?php p($_['edit_url']); ?>" id="editEmployeeForm" enctype="multipart/form-data">
        ...
        <div class="twocontainer">
            ...
            <form method="POST" action="<?php p($_['train_url']); ?>" id="reload-form">
            ...
            <?php if (!empty($_['message']) || !empty($_['error'])): ?>
                <div class="popup-overlay">
                    <div class="popup-box <?php echo !empty($_['error']) ? 'error' : 'success'; ?>">
                        <span class="popup-close" onclick="this.parentElement.parentElement.style.display='none';">×</span>
                        <p><?php echo !empty($_['message']) ? $_['message'] : $_['error']; ?></p>
                    </div>
                </div>
            <?php endif; ?>
        </div> 
