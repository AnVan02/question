...
$ten_khoa = '';
$ma_khoa = '10'; // Change course ID as needed
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['navigate'])) {
        ...
        if ($direction === 'previous' && $current_index > 0) {
            $current_index--;
        } elseif ($direction === 'next' && $current_index < count($_SESSION['questions']) - 1) {
            $current_index++;
        }
...
if (isset($_POST['answer']) && isset($_SESSION['questions'])) {
    ...
    // Move to next question if not navigating
    if (!isset($_POST['navigate']) && $current_index < count($_SESSION['questions']) - 1) {
        $current_index++;
        $_SESSION['current_index'] = $current_index;
    }
}
            ...
            <form method="POST" action="">
                <div class="question-box">
                    ...
                    <div class="navigation-buttons">
                        <button type="submit" name="navigate" value="previous" <?php echo $current_index === 0 ? 'disabled' : ''; ?>>« Câu trước</button>
                        <button type="submit">Trả lời</button>
                        <button type="submit" name="navigate" value="next" <?php echo $current_index === count($_SESSION['questions']) - 1 ? 'disabled' : ''; ?>>Câu tiếp »</button>
                    </div>
